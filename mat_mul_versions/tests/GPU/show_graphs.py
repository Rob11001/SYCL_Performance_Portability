...
labels = ["Naive", "Naive + unroll", "Naive + coarsening", "Naive + coarsening + unroll", "Tiling", "Tiling + unroll", "Tiling + coarsening", "Tiling + coarsening + unroll", "cuBLAS"]
N = 5
avgTimes = {}
stdDeviation = {}
readers = {}
...
for i in [1024, 2048, 4096, 8192]:
    ...
    fig = px.bar(data, y="<b>Version</b>", x="<b>Average Time (ms)</b>", color="<b>Version</b>", title="<b>GPU</b>: {0} x {0}".format(i), text_auto='.2f', orientation="h")
...
for i in [1024, 2048, 4096, 8192]:
    ...
    # Plotly Express
    data = pd.DataFrame(df)
    fig = px.bar(data, y="<b>Version</b>", x="<b>Average Time (ms)</b>", color=" ", title="<b>GPU</b>: {0} x {0}".format(i), text_auto='.2f', orientation="h", barmode="group")
    fig.update_traces(textfont_size=15, textangle=0, textposition="outside", cliponaxis=False)
    fig.update_layout(
        title = {
            'y':0.96,
